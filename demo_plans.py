...
def demonstrate_cost_calculations():
    ...
    print(f"{'Scenario':<15} {'Plan G':<15} {'Plan HDG':<15} {'Difference':<15}")
    print("-" * 60)
    
    # Healthy scenario (premiums only)
    g_healthy = plan_g.calculate_annual_costs(0, False)
    hdg_healthy = plan_hdg.calculate_annual_costs(0, False)
    diff_healthy = hdg_healthy - g_healthy
    
    print(f"{'Healthy':<15} ${g_healthy:<14.2f} ${hdg_healthy:<14.2f} ${diff_healthy:<14.2f}")
    
    # Sick scenario (premiums + deductibles)
    g_sick = plan_g.calculate_annual_costs(0, True)
    hdg_sick = plan_hdg.calculate_annual_costs(0, True)
    diff_sick = hdg_sick - g_sick
    
    print(f"{'Sick':<15} ${g_sick:<14.2f} ${hdg_sick:<14.2f} ${diff_sick:<14.2f}")
    ...
    diff_healthy_5 = hdg_healthy_5 - g_healthy_5
    
    print(f"{'Healthy':<15} ${g_healthy_5:<14.2f} ${hdg_healthy_5:<14.2f} ${diff_healthy_5:<14.2f}")
    
    # Sick scenario year 5
    g_sick_5 = plan_g.calculate_annual_costs(5, True)
    hdg_sick_5 = plan_hdg.calculate_annual_costs(5, True)
    diff_sick_5 = hdg_sick_5 - g_sick_5
    
    print(f"{'Sick':<15} ${g_sick_5:<14.2f} ${hdg_sick_5:<14.2f} ${diff_sick_5:<14.2f}")
...
def demonstrate_monte_carlo_simulation():
    ...
    mean_diff = np.mean(lifetime_hdg) - np.mean(lifetime_g)
    print(f"\nMean difference (HDG - G): ${mean_diff:.2f}")
    
    if mean_diff < 0:
        print(f"Plan HDG is on average ${abs(mean_diff):.2f} cheaper over 25 years")
    else:
        print(f"Plan G is on average ${mean_diff:.2f} cheaper over 25 years")
...
def main():
    ...
    try:
        ...
        print("\nBoth Plan-G and Plan-HDG are now available for use!")
