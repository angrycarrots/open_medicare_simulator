...
def create_plan_selection() -> tuple:
    ...
    plan_type = st.sidebar.selectbox(
        ...
        ["Plan-G (Original Medigap Plan N)", "Plan-HDG (High Deductible Plan G)", "Plan-N (New Plan with Specialist Visits)", "Custom Plan"],
...
def display_plan_summary(plan):
    ...
    # Show specialist visit information if available
    if plan.specialist_visits_per_year is not None:
        st.markdown("**Specialist Visit Information:**")
        col3, col4 = st.columns(2)
        with col3:
            st.write(f"• Visits per Year: {plan.specialist_visits_per_year}")
            st.write(f"• Copay per Visit (2026): ${plan.specialist_copay_2026:.2f}")
        with col4:
            st.write(f"• Copay Growth Rate: {plan.specialist_copay_growth_rate:.1%}")
            annual_specialist_cost = plan.specialist_visits_per_year * plan.specialist_copay_2026
            st.write(f"• Annual Specialist Cost (2026): ${annual_specialist_cost:.2f}")
